...
namespace Skyrim_Alchemy_Utility
{
    public class Ingredient
    {
        ...
        public List<Effect> Effects
        {
            ...
            {
                if (_effects == null)
                    _effects = (from prop in WebScraper.Properties
                        let ingId = prop.IngID
                        let efId = prop.EfID
                        where ingId == ID
                        let ef = WebScraper.Effects[efId]
                        orderby ef.Name
